...
/*
 * Not strictly related to ntof
 */


/*
 * Search parameter from formatted text file, in lines beginning with "# " e.g.
 # This parameter = val1 val2 val3
 *
 * return the value at the given position (starting from 0) after the "="
 */

TString getSPar(TString parname, Int_t idx,
		TString inFile,
		TString defval="-1") {
...
/*
 * convert string time 
 * from: https://stackoverflow.com/questions/17681439/convert-string-time-to-unix-timestamp
 */

// return seconds since epoch of the provided time string 
...
/*
 * return date and time in "ntof" format:
 * two integers as
 *   date: 1YYMMDD
 *   time: mmhhss
 */
...
/*
 * class that read the "summary" NTOF root file generated by ntofData.cpp::ntofData() method from the ntof root file provided by Riccardo (for CERN Oct 2023 tests)
 * if file is not available, it return "default -1" values
 *
 */
class manageNTOF {
 private:
 ...
 public:
  ...
  void setStartDaTi(Long64_t udatetime) { // set start time (reference run time) of fers/srs run under analysis
    start_time = udatetime;
    printf(" Start Time : %lld\n",start_time);
    centry=0;
  }

  // search for the time closest entry in the nTOF tree and return the pulse intensity
  // current_us : fers/srs us timing
  //
  // TIME CONSUMING, need improvement
  //
  //   current_us : trigger time to be searched in NTOF dats
  //   dtc_s: max time difference in sec between correlated trigger timing
  //
  // return:
  ...
  std::vector<float> getBeamPulse(float current_us, float dtc_s=1) {
	...
	mini = delta;
	centry = count;
	if (mini<delta) { break; } // minimum is before 
	if (mini <= dtc_s) { break; } // this should be the matching condition
      }
      count = count +1;
    }
   ...
   *
   */
  int matchedEntry() { return centry; } // return last matched entry
