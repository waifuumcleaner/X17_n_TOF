...
std::vector<std::vector<float>> get_ToT_pedestal(const std::vector<int> act_ch_v, const float def_rms, const std::string basepath, const std::vector<int> ped_runs) {
    ...
    if (file_check.good()) {
        ...
        // std::cout << "Entries for channel " << j << ": " << n << '\n';
        // int iside = c2side(ch);

        // std::cout << "For channel " << j << " iside is " << iside << ", idx is "
        // << idx << ", cid is " << cid << "\n";
        /*if (j % 2 == 0 && j != 28) {
            std::cout << "Coupling channel " << j << " with channel " << j + 1 <<
        '\n'; } else if (j == 28) { std::cout << "Channel " << j << " is alone
        (channel " << j + 1 << " is dead)\n";
        }*/

        // plot in the same graph couples at the end of scintillator bars

        // std::cout << "ped_edge_bin for channel " << j << ": " << ped_edge_bin <<
        // '\n';

        // hped->GetYaxis()->SetRangeUser(0.0001, 3 *
        // hped->GetBinContent(hped->GetMaximumBin()));
        // hcutped->GetYaxis()->SetRangeUser(0.0001, 3 *
        // hped->GetBinContent(hped->GetMaximumBin()));

        // adjust_y_scale(hped, (TPad *)c_ped->cd(couple_number + 1));
        // c_ped->Update();
    };

    // c_temp->SaveAs("ToT.png");
...
summaryList *channelTimeProcess(const int run_number, TTree *tl,
    ...
    //
    //
    coinc_counter counter(run_number, runDelays.at(run_number), bar_coinc_counter, bar_2coinc_counter, bar_cross_coinc_counter);
...
int main() {
    const std::vector<int> run_numbers = {29};
